...
namespace Leguar.TotalJSON.Examples {
	...
	public class ParseAndHandleJSON : MonoBehaviour {
		...
		private void ParseFBUserData(string jsonDataFromFbAsString) {
			...
			Log("isPictureSilhouette = " + isPictureSilhouette);
			Log("pictureUrl = " + pictureUrl);
			Log("country = " + country);
			Log("usdExchange = " + usdExchange);
		...
		private void ParseGPSReceiptData(string jsonDataFromGpsAsString) {
			...
			Log("Store: " + jsonReceipt.GetString("Store"));
			Log("TransactionID: " + jsonReceipt.GetString("TransactionID"));
			...
			Log("packageName: " + receiptAsJSON.GetString("packageName"));
			Log("purchaseTime: " + receiptAsJSON.GetJNumber("purchaseTime").AsLong());

		}
	
		private static void Log(object str) {
			Debug.Log("[ParseAndHandleJSON] " + str);
		}
